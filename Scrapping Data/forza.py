...
if __name__ == '__main__':
    ...
    csv_file_name = "forza_wiki_car_specs.csv" # GANTI DENGAN NAMA FILE ANDA
    ...
    if not os.path.exists(csv_file_name):
        ...
        for index, row in df.iterrows():
            if 'Source URL' not in row or pd.isna(row['Source URL']):
                print(f"\n   [PROSES {index + 1}/{total_cars}] -> ⚠️ Melewati baris karena 'Source URL' kosong.")
                continue

            # --- PERBAIKAN FINAL DI SINI ---
            # Membersihkan URL dari koma atau spasi tambahan di akhir
            page_url = str(row['Source URL']).strip().rstrip(',')
            
            car_name = str(row.get('Merek', '')) + " " + page_url.split('/')[-1].replace('_', ' ')
            
            print(f"\n   [PROSES {index + 1}/{total_cars}] Mobil: {car_name.strip()}")
            ...
            try:
                ...
                if image_tag and image_tag.get('content'):
                    ...
                    file_name = clean_filename(car_name.strip())
